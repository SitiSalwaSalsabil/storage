...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 05 Agu 2024 pada 09.47
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getSiswaByBorn` (IN `input_tempat_lahir` VARCHAR(100))   BEGIN
    SELECT * 
    FROM datasiswa 
    WHERE tempat_lahir = input_tempat_lahir;
END$$
...
-- Fungsi
--
CREATE DEFINER=`root`@`localhost` FUNCTION `getJmlByGender` (`gender` VARCHAR(10)) RETURNS INT(11)  BEGIN
    DECLARE jumlah INT;

    -- Hitung jumlah siswa berdasarkan gender
...
--
DELIMITER $$
CREATE TRIGGER `trg_delete_siswa` AFTER DELETE ON `datasiswa` FOR EACH ROW BEGIN
